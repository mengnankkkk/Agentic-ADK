...
import static com.alibaba.langengine.tencentmap.sdk.TencentMapConstant.IP_LOCATION_API_ENDPOINT;
...
public class TencentMapClient {
	...
	/**
	 * Use Tencent Map API to Location Search. (Full Request) Doc: <a href=
	 * "https://lbs.qq.com/service/webService/webServiceGuide/search/webServiceSearch">...</a>
	...
	/**
	 * Use Tencent Map API to Location Search. (Simple Request) Doc: <a href=
	 * "https://lbs.qq.com/service/webService/webServiceGuide/search/webServiceSearch">...</a>
	 ...
	 *
	 * Through the terminal device IP address to obtain its current geographical location,
	 * commonly used in non-precise positioning scenarios such as displaying local city
	 * weather forecast and initializing user city.
	 *
	 * Notes: 1. IP positioning supports IPv4/IPv6 address positioning. 2. The precision
	 * range of IP positioning is not fixed, with the highest accuracy reaching
	 * district/county level and the lowest to country level. 3. IP positioning cannot
	 * currently cover all IPs, and there may be positioning failures.
	 * @param request IP location request parameters
	 * @return IP location response
	 * @throws TencentMapException when API call fails
	 */
	public IpLocationResponse getIpLocation(IpLocationRequest request) throws TencentMapException {
		try {
			HttpUrl.Builder urlBuilder = HttpUrl.parse(TENCENT_MAP_API_URL + IP_LOCATION_API_ENDPOINT).newBuilder();
			...
			throw new TencentMapException("Error occurred while calling Tencent map IP location API", e);
		}
		catch (Exception e) {
			throw new TencentMapException("Unexpected error occurred while calling Tencent map IP location API", e);
		}
	}

	/**
	 * Get location information based on IP address (simple version)
	 * @param ip IP address (optional, when omitted the requesting IP will be used)
	 * @return IP location response
	 * @throws TencentMapException when API call fails
	 */
	public IpLocationResponse getIpLocation(String ip) throws TencentMapException {
		IpLocationRequest request = new IpLocationRequest();
		request.setIp(ip);
		return getIpLocation(request);
	}
