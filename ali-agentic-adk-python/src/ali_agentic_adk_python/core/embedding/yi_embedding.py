...
class YiEmbedding(BasicEmbedding):
    """Embedding provider backed by 01.AI (Lingyi Wanwu) embeddings API."""

    _DEFAULT_ENDPOINT = "https://api.lingyiwanwu.com/v1/embeddings"
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use YiEmbedding")
        if requests is None:
            raise ImportError("requests is required to use YiEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from Yi provider"
        ...
        try:
            ...
            message = "Failed to parse Yi embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("Yi response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "Yi embedding vector contained non-numeric values",
