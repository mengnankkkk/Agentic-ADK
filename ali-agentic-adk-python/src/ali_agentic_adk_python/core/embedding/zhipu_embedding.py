...
class ZhipuEmbedding(BasicEmbedding):
    """Embedding provider backed by Zhipu AI (GLM) embeddings API."""

    _DEFAULT_ENDPOINT = "https://open.bigmodel.cn/api/paas/v4/embeddings"

    def __init__(
        self,
        api_key: str,
        model: str = "text-embedding-2",
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use ZhipuEmbedding")
        if requests is None:  
            raise ImportError("requests is required to use ZhipuEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from Zhipu provider"
        ...
        try:
            ...
            message = "Failed to parse Zhipu embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("Zhipu response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "Zhipu embedding vector contained non-numeric values",
                original_exception=exc,
            ) from exc


__all__ = ["ZhipuEmbedding"]
