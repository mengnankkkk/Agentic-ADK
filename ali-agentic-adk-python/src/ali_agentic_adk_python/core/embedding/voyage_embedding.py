...
class VoyageEmbedding(BasicEmbedding):
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use VoyageEmbedding")
        if requests is None:  
            raise ImportError("requests is required to use VoyageEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from Voyage provider"
        ...
        try:
            ...
            message = "Failed to parse Voyage embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("Voyage response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "Voyage embedding vector contained non-numeric values",
