...
class DoubaoEmbedding(BasicEmbedding):
    """Embedding provider backed by ByteDance Doubao (Volcengine Ark) embeddings API."""

    _DEFAULT_ENDPOINT = "https://ark.cn-beijing.volces.com/api/v3/embeddings"
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use DoubaoEmbedding")
        if requests is None: 
            raise ImportError("requests is required to use DoubaoEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from Doubao provider"
        ...
        try:
            ...
            message = "Failed to parse Doubao embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("Doubao response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "Doubao embedding vector contained non-numeric values",
