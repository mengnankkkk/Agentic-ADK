...
class MoonshotEmbedding(BasicEmbedding):
    """Embedding provider backed by Moonshot (Kimi) embeddings API."""

    _DEFAULT_ENDPOINT = "https://api.moonshot.cn/v1/embeddings"
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use MoonshotEmbedding")
        if requests is None: 
            raise ImportError("requests is required to use MoonshotEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from Moonshot provider"
        ...
        try:
            ...
            message = "Failed to parse Moonshot embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("Moonshot response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "Moonshot embedding vector contained non-numeric values",
                original_exception=exc,
            ) from exc


__all__ = ["MoonshotEmbedding"]
