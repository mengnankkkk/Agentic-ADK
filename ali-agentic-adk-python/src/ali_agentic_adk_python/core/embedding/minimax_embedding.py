...
class MiniMaxEmbedding(BasicEmbedding):
    """Embedding provider backed by MiniMax embeddings API."""

    _DEFAULT_ENDPOINT = "https://api.minimaxi.com/v1/embeddings"
    ...
    def __init__(
        ...
        *,
        ...
        if not api_key:
            raise ValueError("api_key is required to use MiniMaxEmbedding")
        if requests is None:  
            raise ImportError("requests is required to use MiniMaxEmbedding") from _IMPORT_ERROR
    ...
    def embed_documents(self, texts: Sequence[str]) -> List[List[float]]:
        ...
        try:
            ...
            message = "Failed to retrieve embeddings from MiniMax provider"
        ...
        try:
            ...
            message = "Failed to parse MiniMax embedding response body"
            logger.exception(message)
            raise EmbeddingProviderError(message, original_exception=exc) from exc

        vectors = self._extract_embeddings(content)
        if not vectors:
            raise EmbeddingProviderError("MiniMax response did not contain embedding vectors")
    ...
    def _coerce_vector(vector: Sequence[Any]) -> List[float]:
        try:
            ...
            raise EmbeddingProviderError(
                "MiniMax embedding vector contained non-numeric values",
