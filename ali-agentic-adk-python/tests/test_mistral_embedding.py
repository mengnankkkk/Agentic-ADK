...
from ali_agentic_adk_python.core.embedding.mistral_embedding import MistralEmbedding


class MistralEmbeddingTestCase(unittest.TestCase):
    def test_missing_api_key_raises(self):
        with self.assertRaises(ValueError):
            MistralEmbedding(api_key="")

    @patch("ali_agentic_adk_python.core.embedding.mistral_embedding.requests")
    def test_embed_documents_returns_vectors(self, requests_module):
        ...
        embedding = MistralEmbedding(api_key="token", model="mistral-embed")
        ...
        self.assertEqual(args[0], "https://api.mistral.ai/v1/embeddings")
        self.assertEqual(kwargs["json"], {"model": "mistral-embed", "input": ["hello", "world"]})
    ...
    def test_request_options_are_forwarded(self, requests_module):
        ...
        self.assertEqual(
            kwargs["json"],
            {"model": "custom-model", "input": ["sample"], "encoding_format": "float"},
        )
    ...
    def test_missing_vectors_raise(self, requests_module):
        ...
        embedding = MistralEmbedding(api_key="token")
