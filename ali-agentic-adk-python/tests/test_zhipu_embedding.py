...
from ali_agentic_adk_python.core.embedding.zhipu_embedding import ZhipuEmbedding


class ZhipuEmbeddingTestCase(unittest.TestCase):
    def test_missing_api_key_raises(self):
        with self.assertRaises(ValueError):
            ZhipuEmbedding(api_key="")

    @patch("ali_agentic_adk_python.core.embedding.zhipu_embedding.requests")
    def test_embed_documents_returns_vectors(self, requests_module):
        ...
        embedding = ZhipuEmbedding(api_key="token", model="text-embedding-2")
        ...
        self.assertEqual(args[0], "https://open.bigmodel.cn/api/paas/v4/embeddings")
        self.assertEqual(kwargs["json"], {"model": "text-embedding-2", "input": ["你好", "世界"]})
    ...
    def test_request_options_are_forwarded(self, requests_module):
        ...
        self.assertEqual(
            kwargs["json"],
            {"model": "text-embedding-2", "input": ["示例"], "user_id": "alice"},
        )
        self.assertEqual(kwargs["timeout"], 6.5)
    ...
    def test_missing_vectors_raise(self, requests_module):
        ...
        embedding = ZhipuEmbedding(api_key="token")
