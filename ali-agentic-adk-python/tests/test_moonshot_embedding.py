...
from ali_agentic_adk_python.core.embedding.moonshot_embedding import MoonshotEmbedding


class MoonshotEmbeddingTestCase(unittest.TestCase):
    def test_missing_api_key_raises(self):
        with self.assertRaises(ValueError):
            MoonshotEmbedding(api_key="")

    @patch("ali_agentic_adk_python.core.embedding.moonshot_embedding.requests")
    def test_embed_documents_returns_vectors(self, requests_module):
        ...
        embedding = MoonshotEmbedding(api_key="token", model="moonshot-v1-embedding")
        ...
        self.assertEqual(args[0], "https://api.moonshot.cn/v1/embeddings")
        self.assertEqual(kwargs["json"], {"model": "moonshot-v1-embedding", "input": ["你好", "世界"]})
    ...
    def test_request_options_are_forwarded(self, requests_module):
        ...
        embedding = MoonshotEmbedding(
            api_key="token",
            model="moonshot-custom",
        ...
        requests_module.post.assert_called_once()
        args, kwargs = requests_module.post.call_args
        self.assertEqual(args[0], "https://example.com/v1/embeddings")
        self.assertEqual(
            kwargs["json"],
            {"model": "moonshot-custom", "input": ["示例"], "truncation": "end"},
        )
        self.assertEqual(kwargs["timeout"], 4.2)
    ...
    def test_missing_vectors_raise(self, requests_module):
        ...
        embedding = MoonshotEmbedding(api_key="token")
