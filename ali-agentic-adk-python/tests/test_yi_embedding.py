...
from ali_agentic_adk_python.core.embedding.yi_embedding import YiEmbedding


class YiEmbeddingTestCase(unittest.TestCase):
    ...
    def test_none_api_key_raises(self):
        with self.assertRaises(ValueError):
            YiEmbedding(api_key=None)

    @patch("ali_agentic_adk_python.core.embedding.yi_embedding.requests")
    def test_embed_documents_returns_vectors(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="secret-token", model="yi-text-embedding")
        vectors = embedding.embed_documents(["你好", "世界"])

        self.assertEqual(vectors, [[0.11, 0.21], [0.31, 0.41]])
        requests_module.post.assert_called_once()
        args, kwargs = requests_module.post.call_args
        self.assertEqual(args[0], "https://api.lingyiwanwu.com/v1/embeddings")
        self.assertEqual(
            kwargs["json"],
            {"model": "yi-text-embedding", "input": ["你好", "世界"]},
        )
    ...
    def test_request_options_are_forwarded(self, requests_module):
        ...
        embedding = YiEmbedding(
            api_key="token",
            model="yi-embedding-custom",
            endpoint="https://example.com/embeddings",
            timeout=3.8,
            headers={"X-Test": "value"},
            request_options={"truncate": "end"},
        )
        ...
        self.assertEqual(
            kwargs["json"],
            {"model": "yi-embedding-custom", "input": ["示例"], "truncate": "end"},
        )
    ...
    def test_missing_vectors_raise(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="token")
    ...
    def test_default_model_parameter(self, requests_module):
        ...
        args, kwargs = requests_module.post.call_args
        self.assertEqual(kwargs["json"]["model"], "text-embedding-01")
    ...
    def test_tuple_timeout_parameter(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="token", timeout=(5.0, 10.0))
        embedding.embed_documents(["test"])
    ...
    def test_none_timeout_parameter(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="token", timeout=None)
        embedding.embed_documents(["test"])
    ...
    def test_request_options_copied_not_mutated(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="token", request_options=original_options)
    ...
    def test_authorization_header_format(self, requests_module):
        ...
        embedding = YiEmbedding(api_key="my-secret-key-123")
        embedding.embed_documents(["test"])

        args, kwargs = requests_module.post.call_args
        self.assertEqual(kwargs["headers"]["Authorization"], "Bearer my-secret-key-123")

    @patch("ali_agentic_adk_python.core.embedding.yi_embedding.requests")
    def test_content_type_and_accept_headers(self, requests_module):
        response_mock = MagicMock()
        response_mock.raise_for_status.return_value = None
        response_mock.json.return_value = {"data": [{"embedding": [0.1]}]}
        requests_module.post.return_value = response_mock
    ...
    def test_model_attribute_accessible(self, requests_module):
        embedding = YiEmbedding(api_key="token", model="custom-model-name")
        
        self.assertEqual(embedding.model, "custom-model-name")
